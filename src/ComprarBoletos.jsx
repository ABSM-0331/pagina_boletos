...
const ComprarBoletos = () => {
    ...
    // 1. Cargar orígenes
    useEffect(() => {
        fetch("https://api-boletos-production-1679.up.railway.app/origens")
            .then((r) => r.json())
            .then(setOrigenes);
    }, []);
    ...
    useEffect(() => {
        if (origen) {
            fetch(
                `https://api-boletos-production-1679.up.railway.app/rutas?origen=${encodeURIComponent(
                    origen
                )}`
            )
                .then((r) => r.json())
    ...
    useEffect(() => {
        ...
        if (rutaSeleccionada) {
            fetch(
                `https://api-boletos-production-1679.up.railway.app/horarios?id_ruta=${rutaSeleccionada}`
            )
                .then((r) => r.json())
                .then(setHorarios);
        } else {
    ...
    useEffect(() => {
        if (rutaSeleccionada && horario) {
            fetch(
                `https://api-boletos-production-1679.up.railway.app/precio?id_ruta=${rutaSeleccionada}&id_horario=${horario}`
            )
                .then((r) => r.json())
                .then((data) => {
    ...
    const confirmarPago = () => {
        ...
        fetch("https://api-boletos-production-1679.up.railway.app/ventas", {
            method: "POST",
